    ...
    component: {
      ...
      data: {
        loader: async () => {
          ...
          if (hasPluginCades) {
            const result = await Promise.all([getUserCertificates(), getCert(http, id)])
...
const getCert = async (http, id) => {
  try {
    ...
    const { data } = await http.get('asHelper/certificate', { params: { id } }, { signal: abortController.signal })
