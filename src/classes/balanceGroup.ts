...
export class BalanceGroup extends BaseObject {
  ...
  add(
    ...
    this.balanceGroups.push(
      ...items
        .filter(r => r.type === 28)
        .map(r => new BaseBalanceGroup({ id: r.id, name: r.originName, isSource, state: r.state, hasArchives: r.hasArchives, subGroup: r.subGroup }))
    )
    this.nodes.push(
      ...items
        .filter(r => r.type === 19)
        .map(r => new Node({ id: r.id, name: r.name, isSource, nodeType: r.nodeType, state: r.state, hasArchives: r.hasArchives, subGroup: 1 }))
    )
    this.points.push(
      ...items
        .filter(r => r.type === 26)
        .map(r => new Point({ id: r.id, name: r.name, isSource, state: r.state, hasArchives: r.hasArchives, systemType: r.systemType }))
    )
    this.pointLists.push(
      ...items.filter(r => r.type === 15).map(r => new PointList({ id: r.id, name: r.name, isSource, state: r.state, hasArchives: r.hasArchives }))
    )
  }

  remove(items: Array<BaseBalanceGroup | Node | Point | PointList>, isSource: boolean) {   
    items.forEach(r => {
      if (r instanceof BaseBalanceGroup) {
        this.balanceGroups = this.balanceGroups.filter(item => !(r.id === item.id && item.isSource === isSource))
      }
      if (r instanceof Node) {
        this.nodes = this.nodes.filter(item => !(r.id === item.id && item.isSource === isSource))
      }
      if (r instanceof Point) {
        this.points = this.points.filter(item => !(r.id === item.id && item.isSource === isSource))
      }
      if (r instanceof PointList) {
        this.pointLists = this.pointLists.filter(item => !(r.id === item.id && item.isSource === isSource))
      }
    })
  }
}
