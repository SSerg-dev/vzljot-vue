...
export class SystemNodeError {
  ...
  constructor({ name = '', internalAddress = '' }: { name?: string; internalAddress?: string }) {
...
export class SystemNode extends BaseObject {
  ...
  constructor({
    ...
    const unknown = this.endPoints.find(r => r.id === redirectEndPointIdUnknown)

    this.redirectEndPointProperties = new RedirectEndPointProperties({ redirectEndPointIdKnown: known ? known.uuid : undefined, redirectEndPointIdUnknown: unknown ? unknown.uuid : undefined })
  }
  ...
  async init(id: number): Promise<void> {
    ...
    if (SystemNode.store.state.user?.userRights.allowRedirectEndpoints) {
      ...
      const known = this.endPoints.find(r => r.id === data.redirectEndPointIdKnown)
      const unknown = this.endPoints.find(r => r.id === data.redirectEndPointIdUnknown)
  ...
  async save(): Promise<void> {
    ...
    const payload: { [key: string]: any } = {
      ...
      redirectKnown: this.endPoints.find(r => r.uuid === this.redirectEndPointProperties.redirectEndPointIdKnown)?.name,
      redirectUnknown: this.endPoints.find(r => r.uuid === this.redirectEndPointProperties.redirectEndPointIdUnknown)?.name,
    }
    ...
    if (this.groups) {
      ...
      payload.groups = Object.entries(this.groups).map(([k, v]) => ({ group: match[k], value: v}))
    }
  ...
  removeRedirectEndpoints(uuids: Array<string>): void {
    ...
    if (this.redirectEndPointProperties.redirectEndPointIdKnown !== undefined && uuids.includes(this.redirectEndPointProperties.redirectEndPointIdKnown)) this.redirectEndPointProperties.redirectEndPointIdKnown = undefined
    if (this.redirectEndPointProperties.redirectEndPointIdUnknown !== undefined && uuids.includes(this.redirectEndPointProperties.redirectEndPointIdUnknown)) this.redirectEndPointProperties.redirectEndPointIdUnknown = undefined
  }
}
