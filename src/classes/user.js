...
export default class User extends BaseObject {
  constructor(obj) {
    ...
    this.domain = obj && Object.prototype.hasOwnProperty.call(obj, 'domain') ? obj.domain : null

    this.hasPassword = obj && Object.prototype.hasOwnProperty.call(obj, 'hasPassword') ? obj.hasPassword : false

    this.password = obj && Object.prototype.hasOwnProperty.call(obj, 'password') ? obj.password : this.hasPassword ? password : ''

    this.enabled = obj && Object.prototype.hasOwnProperty.call(obj, 'enabled') ? obj.enabled : true

    this.groups = obj && Object.prototype.hasOwnProperty.call(obj, 'groups') ? obj.groups : User.store.state.env.userGroups

    this.userGroups = obj && Object.prototype.hasOwnProperty.call(obj, 'userGroups') ? this.groups.filter(r => obj.userGroups.includes(r.id)).map(r => r.id) : this.groups.filter(r => r.typeCode === 'user').map(r => r.id)

    this.desc = obj && Object.prototype.hasOwnProperty.call(obj, 'desc') ? obj.desc : null

    this.limitEquips = obj && Object.prototype.hasOwnProperty.call(obj, 'limitEquips') ? obj.limitEquips : false

    this.limitPoints = obj && Object.prototype.hasOwnProperty.call(obj, 'limitPoints') ? obj.limitPoints : false

    this.limitReports = obj && Object.prototype.hasOwnProperty.call(obj, 'limitReports') ? obj.limitReports : false
    ...
    this.includeRelatedPoints = obj && Object.prototype.hasOwnProperty.call(obj, 'includeRelatedPoints') ? obj.includeRelatedPoints : false
    ...
    this.view =
      obj && obj.view
        ...
        : {
            ...
            timeType: User.matchType(User.store.state.env.pollDataTimeTypes).onDepth,
      ...
      obj.balanceGroups.forEach(r => {
        if (r.groups & 3) {
          if (r.groups === 1 || r.groups === 3) {
            this.balanceGroups.push({ id: r.id, type: r.type, name: r.name + ' (Источники)', groups: 1, state: r.state, checked: false })
          }
          if (r.groups === 2 || r.groups === 3) {
            this.balanceGroups.push({ id: r.id, type: r.type, name: r.name + ' (Потребители)', groups: 2, state: r.state, checked: false })
          }
        }
      })
    }
  ...
  async save(userRights, subscriptions) {
    ...
    let obj = {
      ...
      balanceGroups: Object.entries(balanceGroups).map(([key, value]) => ({ id: parseInt(key), value })),
      notification: {
      ...
      view: {
        ...
        timeDepth: User.matchType(User.store.state.env.pollDataTimeTypes).onDepth === this.view.timeType ? this.view.timeDepth : null,
        timeFrom: User.matchType(User.store.state.env.pollDataTimeTypes).fromTime === this.view.timeType ? new Date(this.view.timeFrom).getTime() : null,
        timeTo: User.matchType(User.store.state.env.pollDataTimeTypes).fromTime === this.view.timeType ? (this.view.isTimeTo ? new Date(this.view.timeTo).getTime() : null) : null,
        useWebSystem: this.view.useWebSystem.visible ? this.view.useWebSystem.value : null,
        info: this.view.info,
    ...
    if (userRights && Object.keys(userRights) && userRights.constructor === Object) {
    ...
    if (subscriptions && subscriptions.constructor === Array) {
      ...
      obj.subscriptions = JSON.parse(JSON.stringify(subscriptions, getCircularReplacer()))
      ...
      obj.subscriptions.forEach(r => {
        ...
        r.balanceGroups = Object.entries(bg).map(([key, value]) => ({ id: parseInt(key), value }))
        r.nodes = r.nodes.map(x => x.id)
        r.events = Object.entries(r.events).map(([key, value]) => ({ messageType: key, source: value.source, selected: Array.isArray(value.selected) && value.selected.length === 0 ? null : JSON.stringify(value.selected) }))
      })
    }
    ...
    const { data } = await this.http.post('user/user', obj)

    return data
  }

  async testEmail() {
    return await this.http.get('user/testEmail', { params: { email: this.contactEmail } })
  }

  async testSms() {
    return await this.http.get('user/testSms', { params: { phone: this.contactPhone } })
  }
}
