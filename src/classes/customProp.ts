...
export class CustomPropError {
  ...
  constructor({ name = '', code = '', dataType = '', objectType = '' }: { name?: string; code?: string; dataType?: string; objectType?: string }) {
...
export class CustomProp extends BaseObject {
  ...
  async init(id: number) {
    if (!Number.isNaN(id)) {
      const { data } = await this.http.get<CustomProp>('customProps/customProp', { params: { id } })
  ...
  async save() {
    await this.http.post('customProps/customProp', {
      id: this.id,
      name: this.name,
      code: this.code,
