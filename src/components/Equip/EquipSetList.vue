<template>
  <div class="component-detail">
    <tool-bar :exportTypes="viewType === 'table' ? ['csv', 'xls', 'xlsx'] : ['jpeg', 'png', 'svg']" @export="exportData">
      <label
        >с:
        <date-picker v-model="periodStart" :format="dateFormat" style="width: 150px" />
      </label>
      <label
        >по:
        <date-picker v-model="periodEnd" :format="dateFormat" style="width: 150px" />
      </label>
      <label
        >Данные:
        <select v-bind:value="pollType" @change="onPollTypeChange($event.target.value)">
          <option v-if="hasEquipEvents" :value="parseInt($store.getters.pollDataTypes.equipEvents)">{{ $store.state.env.pollDataTypes[$store.getters.pollDataTypes.equipEvents].text }}</option>
          <option :value="parseInt($store.getters.pollDataTypes.setParams)">{{ $store.state.env.pollDataTypes[$store.getters.pollDataTypes.setParams].text }}</option>
      ...
      <template v-slot:end>
        <div :class="['button', 'fas', 'fa-cog', { disabled: measures.length === 0 || !measures.some(r => r.items.length > 1) }]" title="Настройки" @click="onSettingsClick" />
      </template>
    </tool-bar>
    <template v-if="viewType === 'table'">
      <div class="table-grid" :style="`grid-template-columns: repeat(${columns.length}, max-content)`">
        <div class="table-row">
          <header v-for="(r, i) in columns" :key="i" :class="['header', 'right', { 'sticky-first': i === 0 }, { active: i === sortIndex }]" :title="r.title">
            <template v-if="r.hasOwnProperty('sort')">
              <span style="flex: 1">{{ r.text }}</span>
              <span :title="sortTitle[r.sort]" :class="['arrow', 'fas', sortAccordance[r.sort]]" @click="onSort(r.sort === 'desc' ? 'asc' : 'desc', i)"></span>
            </template>
            <template v-else>
              {{ r.text }}
            </template>
        ...
        <div class="table-row" v-for="(row, rowIndex) in localItems" :key="rowIndex">
          <span :class="['cell', 'right', { hint: hasHint(r) }, { 'sticky-first': i === 0 }]" v-for="(r, i) in row" :key="(rowIndex + 1) * (i + 1)" :title="getCellTitle(r)">{{ getCellValue(r, columns[i] ? columns[i].type : null) }}</span>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      viewType: 'table',
      pollType: this.hasEquipEvents ? this.$store.getters.pollDataTypes.equipEvents : this.$store.getters.pollDataTypes.setParams,
  ...
  methods: {
    allowChart()
    {
      return !(this.pollType === this.$store.getters.pollDataTypes.equipEvents || this.pollType === this.$store.getters.pollDataTypes.equipCustomizing)
    },
    sort() {
      return true
    },
    ...
    async load() {
      ...
      try {
        ...
        this.measures = [...new Set(data.columns.filter(r => r.hasOwnProperty('mu')).map(r => this.$store.state.env.mUnits[r.mu]))]
          .reduce((acc, r) => {
    ...
    async exportData(type) {
      ...
      model.fileName = `${this.equipName}, ${this.$store.state.env.pollDataTypes[this.pollType].text} (${periodStart} - ${periodEnd})`
