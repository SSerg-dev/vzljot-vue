<template>
  <div class="component-detail">
    <tool-bar>
      <div v-if="$store.state.user?.userRights.customPropertyEdit" :class="['button', 'fas', 'fa-plus-circle', { disabled: wait }]" title="Добавить..." @click="onAddClick" />
      <div v-if="$store.state.user?.userRights.customPropertyEdit" :class="['button', 'fas', 'fa-times-circle', { disabled: !hasSelected || wait }]" title="Удалить" @click="onRemoveClick" />
      <template v-slot:end>
    ...
    <div class="table-grid" :style="{ 'grid-template-columns': `repeat(7, max-content)` }">
      <header class="header" />
      <header class="header">
        <input type="checkbox" v-model="all" @change="changeAll" :disabled="wait" />
      </header>
      ...
      <div v-for="r in localItems" class="table-row" :key="r.id">
        <span class="cell icon">
          <div :class="'fas fa-brackets-curly clickable-icon'" @click="viewClick(r)" title="Просмотр..." />
        </span>
        <span class="cell"><input type="checkbox" v-model="r.checked" :disabled="wait"/></span>
...
export default {
  ...
  computed: {
    filteredItems() {
      ...
      if (this.filter.name !== null) {
        customProps = customProps.filter(r => r.name.toLowerCase().includes(this.filter.name.toLowerCase()))
      }

      if (this.filter.code !== null) {
        customProps = customProps.filter(r => r.code.toLowerCase().includes(this.filter.code.toLowerCase()))
      }

      if (this.filter.dataType !== null) {
        customProps = customProps.filter(r => r.dataType === this.filter.dataType)
      }

      if (this.filter.objectType !== null) {
        customProps = customProps.filter(r => r.objectType === this.filter.objectType)
      }
  ...
  methods: {
    displayIn(value) {
      return [(CustomPropertyDisplayInEnum.List & value) === CustomPropertyDisplayInEnum.List ? 'Списки' : '', (CustomPropertyDisplayInEnum.Counter & value) === CustomPropertyDisplayInEnum.Counter ? 'Показания счетчиков' : ''].filter(r => r !== '').join('; ')
    },
    objectType(type) {
      if (type === CustomPropertyObjectTypeEnum.Equip) return 'Прибор'
      if (type === CustomPropertyObjectTypeEnum.MeasureScheme) return 'Точка учета'
      if (type === CustomPropertyObjectTypeEnum.EquipAndMeasureScheme) return 'Прибор и точка учета'
    },
    propertyType(type) {
