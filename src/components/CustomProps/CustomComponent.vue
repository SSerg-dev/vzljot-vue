<template>
  <div class="component-detail">
    <preserver-component
      v-bind="{ readOnly: !$store.state.user?.userRights.customPropertyEdit, saving, disabled: !hasChanges || loading, loading }"
      @saveClick="save()"
    >
      <custom-prop-component ref="customProp" v-bind="{ value: localItem, error: localError }" @changed="onChanged" />
    </preserver-component>
    <transition-group>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const { hasChanges, loading, localItem, localError, onChanged, onWizardCancel, onWizardEnd, save, saving, wizard } = setupComponent<
      CustomProp,
      CustomPropError
    >(new CustomProp({ id: props.id, uuid: props.uuid }), new CustomPropError({}), emit)
