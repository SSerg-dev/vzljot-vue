<template>
  <div class="component-detail">
    <tool-bar>
      <div :class="['button', 'fas', 'fa-envelope', { disabled: !hasSelected || wait }]" title="Квитировать" @click="onConfirmClick()" />
      <div :class="['button', 'fas', 'fa-times-circle', { disabled: !hasSelected || wait }]" title="Удалить" @click="onRemoveClick()" />
      <template v-slot:end>
        <label>
          <input type="checkbox" v-model="isConfirm" style="cursor: pointer;" />
          Не квитированные
        </label>
    ...
    <div class="table-grid" :style="{ 'grid-template-columns': `repeat(${$store.state.user?.userRights.analyse ? 9 : 8}, max-content)` }">
      <header v-if="$store.state.user?.userRights.analyse" class="header"></header>
      ...
      <div v-for="(r, i) in localItems" :key="i" class="table-row">
        <span v-if="$store.state.user?.userRights.analyse" class="cell icon">
          <div :class="r.hasAnalyze ? 'fas fa-chart-pie clickable-icon' : ''" @click="viewClick(r)" title="Просмотр..." />
        </span>
        <span class="cell">
          <input type="checkbox" v-model="r.checked" :disabled="wait" />
        </span>
        <span :class="['cell', { isConfirm: r.isConfirm }]">{{ new Date(r.timeRecive).toLocaleString() }}</span>
        <span :class="['cell', { isConfirm: r.isConfirm }]">{{ r.objectTypeText }}</span>
        <span :class="['cell', { isConfirm: r.isConfirm }]">{{ r.objectText }}</span>
        <span :class="['cell', { isConfirm: r.isConfirm }]">{{ r.message }}</span>
        <span :class="['cell', { isConfirm: r.isConfirm }]">{{ r.status }}</span>
        <span :class="['cell', { isConfirm: r.isConfirm }]">{{ r.timeEvent ? new Date(r.timeEvent).toLocaleString() : '' }}</span>
        <span :class="['cell', { isConfirm: r.isConfirm }]">{{ r.sourceText }}</span>
      </div>
    </div>
...
export default {
  components: {
    ...
    Temperature: asyncImport(() => import('../Analyze/TemperatureComponent.vue')),
    Props,
    Pager,
    ToolBar,
    Wizard
  ...
  methods: {
    ...
    async viewClick(r) {
      this.wait = true
      try {
        const { data } = await this.$http.get('systemMessage/message', { params: { id: r.messageId } })

        this.component = {
          name: data.component,
          data: {
            id: data.id,
            timeStart: r.timeEvent ? new Date(r.timeEvent) : new Date(new Date().getFullYear(), new Date().getMonth() - 1, new Date().getDate(), 0, 0, 0),
    ...
    async get(page, size) {
      ...
      try {
        ...
        const { data } = await this.$http.post('systemMessage/messages', model, {
    ...
    onWizardEnd(data, name) {
      ...
      if (name === 'remove') {
        ...
        this.confirm(this.dataItems.filter(r => r.checked === true).map(r => r.id))
