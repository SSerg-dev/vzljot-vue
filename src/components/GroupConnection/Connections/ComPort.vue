<template>
  <div class="connection-type">
    ...
    <div class="connection-type two">
      <template v-if="group.type === connectionTypes.comPort_RS232 || group.type === connectionTypes.comPort_RS485">
        <div class="connection-type two">
          <label>Порт:</label>
      ...
      <template v-if="group.type === connectionTypes.comPort_RSEthernet">
        ...
        <div class="connection-type two">
          <label>Порт:</label>
          <number-box @update:modelValue="onChange('networkPort', $event)" v-model="localNetworkPort" :min="0" :max="65535" :class="{ 'validation-error': localError.networkPort }" :title="localError.networkPort" style="width: 60px" />
        </div>
      </template>
    </div>
    <div class="connection-type two">
      <template v-if="group.type === connectionTypes.comPort_RSEthernet">
        ...
        <select @input="onChange('networkProtocol', parseInt($event.target.value))" v-model="localNetworkProtocol">
          <option v-for="key in Object.keys($store.state.env.networkProtocols)" :key="key" :value="parseInt(key)">{{ $store.state.env.networkProtocols[key].text }}</option>
        </select>
      </template>
