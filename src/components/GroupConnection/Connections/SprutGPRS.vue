<template>
  <div class="connection-type">
    <div class="connection-type two">
      ...
      <input @input="onChange('adapter', $event.target.value)" v-model="localAdapter" type="text" :class="{ 'validation-error': localError.adapter }" :title="localError.adapter" maxlength="15" style="width: 130px" />
    </div>
    <div class="connection-type two">
    ...
    <div class="connection-type two">
      <div class="connection-type two">
        ...
        <select @change="onChange('type', localType)" v-model="localType">
          <option :value="connectionTypes.bitCordSprutNet_Modem">Через модем</option>
          <option :value="connectionTypes.bitCordSprutNet_GPRS_WithoutCall">Без вызова</option>
        </select>
      </div>
      <div v-if="group.type === connectionTypes.bitCordSprutNet_Modem" class="connection-type two">
        <div class="connection-type two">
          <label>Порт:</label>
