<template>
  <div class="connection-type">
    <div class="connection-type two">
      <label>IMEI адаптера (15 цифр):</label>
      <input @input="onChange('adapter', $event.target.value)" v-model="localAdapter" type="text" :class="{ 'validation-error': localError.adapter }" :title="localError.adapter" maxlength="15" />
    </div>
    <div class="connection-type two">
    ...
    <div class="connection-type two">
      <div class="connection-type two">
        ...
        <select @change="onChange('type', localType)" v-model="localType">
          <option :value="connectionTypes.TELEOFIS_WRX">Через модем</option>
          <option :value="connectionTypes.TELEOFIS_WRX_SIP">Через VoIP</option>
          <option :value="connectionTypes.TELEOFIS_WRX_WithoutCall">Без вызова</option>
        </select>
      </div>
      <div v-if="group.type === connectionTypes.TELEOFIS_WRX" class="connection-type two">
        <div class="connection-type two">
          <label>Порт:</label>
      ...
      <div v-else-if="group.type === connectionTypes.TELEOFIS_WRX_SIP" class="connection-type two">
        <label :disabled="group.type !== connectionTypes.TELEOFIS_WRX_SIP">SIP-аккаунт:</label>
        <select @change="onChange('sip', localSip)" v-model="localSip" :disabled="group.type !== connectionTypes.TELEOFIS_WRX_SIP">
