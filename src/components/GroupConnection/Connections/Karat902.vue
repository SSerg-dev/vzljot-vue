<template>
  <div class="connection-type">
    <div class="connection-type two">
      ...
      <input @input="onChange('adapter', $event.target.value)" v-model="localAdapter" type="text" :class="{ 'validation-error': localError.adapter }" :title="localError.adapter" maxlength="12" />
    </div>
    <div class="connection-type two">
    ...
    <div class="connection-type two">
      <div class="connection-type two">
        ...
        <select @change="onChange('type', parseInt($event.target.value))" v-model="localType">
          <option :value="connectionTypes.karat902">Через модем</option>
          <option :value="connectionTypes.karat902_SIP">Через VoIP</option>
          <option :value="connectionTypes.karat902_WithoutCall">Без вызова</option>
        </select>
      </div>
      <div v-if="group.type === connectionTypes.karat902" class="connection-type two">
        <div class="connection-type two">
          <label>Порт:</label>
      ...
      <div v-else-if="group.type === connectionTypes.karat902_SIP" class="connection-type two">
