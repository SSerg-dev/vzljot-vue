<template>
  <div class="component-detail">
    ...
    <div class="table-grid" :style="{ 'grid-template-columns': `repeat(6, max-content)` }">
      <header class="header" />
      ...
      <div v-for="(r, index) in localItems" :key="index" class="table-row">
        <span class="cell icon">
          <span :class="getClass(r)" :style="getStyle(r)" title="Просмотр..." @click="viewClick(r)" />
        </span>
    ...
    <transition-group>
      ...
      <props-component v-if="edit" v-bind="component" @close="close">
        <schema-component :key="component.uuid" v-bind="component.data" @saved="onSaved" />
      </props-component>
    </transition-group>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getStyle(item) {
      return {
        position: 'relative',
        color: item.purposeGroup === this.setTypes.none ? lightenDarken('#778899', 25) : '#778899'
      }
    },
    getClass(item) {
      return `fas fa-project-diagram clickable-icon ${item.purposeGroup === this.setTypes.equipGroup ? 'sign-group' : ''}`
    },
    getPurpose(r) {
      return r.purposeGroup === this.setTypes.none ? 'Шаблон' : this.$store.state.env.setTypes[r.purposeGroup].text
    },
    getForAll(r) {
      return r.purposeGroup === this.setTypes.equipType ? (r.availableForAll ? 'Да' : 'Нет') : ''
    },
    emptyFilter() {
      return {
        purposeGroup: null,
