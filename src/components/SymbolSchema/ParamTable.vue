<template>
  <div :title="item.ToolTip" :style="item.getStyle()">
    <tool-bar :exportTypes="['csv', 'xls', 'xlsx']" @export="exportTable(item, $event)">
      <div style="display:flex;flex:1;align-items:center;text-align:center;font-weight:bold;">
        {{ item.DefaultText }}
      </div>
    </tool-bar>
    <div class="table-grid" :style="{ 'grid-template-columns': `repeat(${item.Items.length + 1}, max-content)` }">
      <header class="header">{{ columnDate.text }}</header>
      <header class="header" v-for="(r, i) in item.Items" :key="i">{{ r.Text }}</header>
      <div v-for="(r, rowIndex) in rows" :key="rowIndex" class="table-row">
        <span class="cell">{{ r.time.toLocaleString() }}</span>
        <span class="cell" v-for="(column, i) in item.Items" :key="i">{{ r[column.ParamCode] }}</span>
      </div>
    </div>
  </div>
</template>
    ...
    async exportTable(table, type) {
      ...
      try {
        await this.exporter.post('symbolSchema/exportTable', {
          ...
          values: [...this.rows.map(r => [r.time.toLocaleString(), ...table.Items.map(item => r[item.ParamCode])])],
      ...
      if (message.data.action === 'pollSymbolSchema' && message.data.key === this.item.x.key) {
        ...
        if (this.startTime < time) {
          this.item.Items.filter(r => r.ParamCode === message.data.paramId && (r.EquipID === -1 || r.EquipID === message.data.equipId)).forEach(r => {
            ...
            r.Text = `${r.DefaultText}${r.UseSpServerUnit && mu ? r.Separator + mu.text : ''}`
