...
export default {
  ...
  computed: {
    filteredItems() {
      ...
      if (this.filter.node) {
        rows = rows.filter(r => r.node.toLowerCase().indexOf(this.filter.node.toLowerCase()) >= 0)
      }

      if (this.filter.name) {
        rows = rows.filter(r => r.name.toLowerCase().indexOf(this.filter.name.toLowerCase()) >= 0)
      }
  ...
  methods: {
    ...
    async load(id, objType) {
      try {
        ...
        if (type === 'points') {
          ...
          const { data } = await this.$http.get('address/points', { params: { id } })
          this.dataItems = data
        }
    ...
    loadNode(point) {
      window.open(`${window.props.baseUrl}Node/NodeDetailAlone/${point.nodeId}`, '_blank')
    }
  }
}
</script>
