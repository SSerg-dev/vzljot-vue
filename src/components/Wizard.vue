<template>
  <modal @cancel="onCancel()" :background="background" :cancellable="isCancellable">
    ...
    <template v-slot:content>
      <div style="position: relative">
        <div class="component-detail" style="overflow: visible">
          <component ref="component" :key="currentComponent.key" :is="currentComponent.component" v-bind="currentComponent.data" @[currentComponent.event]="onComponentEvent" @loaded="onLoaded" @mounted="onMounted" />
        </div>
        <spinner :show="loading || wait" :text="waitText" />
      </div>
    </template>
    <template v-slot:footer>
      <button :disabled="nextDisabled" @click="next()">{{ hasNext ? 'Далее' : 'Ok' }}</button>
...
export default {
  ...
  components: {
    ...
    AddressHouseProps: asyncImport(() => import('./Address/AddressHouseProps.vue')),
    ArchiveSettings: asyncImport(() => import('./Archive/ArchiveSettings.vue')),
    BalanceGroupDetail: asyncImport(() => import('./BalanceGroup/BalanceGroupDetail.vue')),
    BalanceGroupItemsFilter: asyncImport(() => import('./BalanceGroup/BalanceGroupItemsFilter.vue')),
    CertPassword: asyncImport(() => import('./Users/CertPassword.vue')),
    CustomPropsFilter: asyncImport(() => import('./CustomProps/CustomPropsFilter.vue')),
    CustomProp: asyncImport(() => import('./CustomProps/CustomProp.vue')),
    EquipDetail: asyncImport(() => import('./Equip/EquipDetail.vue')),
    EquipListFilter: asyncImport(() => import('./EquipList/EquipListFilter.vue')),
    ...
    EquipTypeModificationProps: asyncImport(() => import('./EquipType/ModificationProps.vue')),
    EquipTypeProps: asyncImport(() => import('./EquipType/EquipTypeProps.vue')),
    EquipTypesFilter: asyncImport(() => import('./EquipType/EquipTypesFilter.vue')),
    FileFilter: asyncImport(() => import('./FileList/FileFilter.vue')),
    FormParams: asyncImport(() => import('./ReportForm/FormParams.vue')),
    GroupConnectionDetail: asyncImport(() => import('./GroupConnection/GroupConnectionDetail.vue')),
    ...
    LoraServerProps: asyncImport(() => import('./SystemNode/LoraServerProps.vue')),
    Measures: asyncImport(() => import('./MeasuresComponent.vue')),
    Message,
    Modal,
    ModemProps: asyncImport(() => import('./SystemNode/ModemProps.vue')),
    ModemPoolProps: asyncImport(() => import('./SystemNode/ModemPoolProps.vue')),
    NodeDetail: asyncImport(() => import('./Node/NodeDetail.vue')),
    PollDataSettings: asyncImport(() => import('./Poll/PollDataSettings.vue')),
    PointGroupDetail: asyncImport(() => import('./PointGroup/PointGroupDetail.vue')),
    PointListFilter: asyncImport(() => import('./PointList/PointListFilter.vue')),
    ...
    RedirectEndPointProps: asyncImport(() => import('./SystemNode/RedirectEndPointProps.vue')),
    ReportFileListFilter: asyncImport(() => import('./ReportFile/ReportFileListFilter.vue')),
    ...
    ReportTaskProps: asyncImport(() => import('./ReportTask/ReportTaskProps.vue')),
    ReportTasksFilter: asyncImport(() => import('./ReportTask/ReportTasksFilter.vue')),
    Selector,
    SelectorOption,
    ...
    SipAccountProps: asyncImport(() => import('./SystemNode/SipAccountProps.vue')),
    StatesFilter: asyncImport(() => import('./States/StatesFilter.vue')),
    SymbolSchemasFilter: asyncImport(() => import('./SymbolSchema/SymbolSchemasFilter.vue')),
    SystemMessagesFilter: asyncImport(() => import('./SystemMessages/SystemMessagesFilter.vue')),
    ...
    UserSubscriptionProps: asyncImport(() => import('./Users/UserSubscriptionProps.vue')),
  ...
  computed: {
    ...
    nextDisabled() {
      if (Object.prototype.hasOwnProperty.call(this.currentComponent, 'isLast')) {
      ...
      return this.currentValue === null || (Array.isArray(this.currentValue) && this.currentValue.length === 0 ? true : false)
    },
    isCancellable() {
  ...
  methods: {
    initComponent(value) {
      return Object.assign({ cancellable: true, event: null, key: new Date().getTime() }, value)
    },
    onMounted() {
      this.loading = true
    },
    ...
    async next() {
      ...
      if (this.hasNext) {
        try {
          this.loading = true
          this.currentComponent = this.initComponent(await this.currentComponent.next(this.currentValue))
        } catch (error) {
        ...
        this.$emit('end', this.currentValue, this.name, this.currentComponent.component)
      }
