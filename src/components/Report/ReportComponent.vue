<template>
  <div class="wrapper">
    ...
    <transition>
      <wizard v-if="viewWizard" v-bind="wizard" @cancel="onWizardCancel" @end="onWizardEnd" :background="background" :cancellable="localCancellable" />
    </transition>
  ...
  if (reportTypes) {
    if (reportTypes.length > 0) {
      if (reportTypes.length > 1) {
        ...
        data.component = reportParams(items, objType, store.state.env.reportTypes[reportTypes[0]].type)
      }
    }
  }
...
export default {
  ...
  created() {
    this.$watch(() => this.reportTypes, () => {
      this.wizardFilter = wizardParams(this.ids, this.objType, this.reportTypes)
  ...
  methods: {
    async sign(parameters, certificate) {
      ...
      try {
        if (certificate.isLocal) {
          const res = await this.$http({ url: 'report/reportPdf', data: parameters, method: 'POST', responseType: 'blob' })
          ...
          formData.set('periodStart', getISODateTime(new Date(parameters.periodStart)))
          formData.set('periodEnd', getISODateTime(new Date(parameters.periodEnd)))
          ...
          const { data } = await this.$http.post('report/signCert', Object.assign(parameters, { certificate }), {
    ...
    onWizardCancel(data) {
      ...
      if (data && data.type && this.$store.state.env.resultTypes[data.type].type === 'error') {
        this.$store.commit('notification', {
      ...
      if (wizardName === 'reportWizard') {
        ...
        try {
          ...
          if (componentName === 'report-params') {
            if (this.$store.state.user?.userRights.allowCryptSign) {
              const { data } = await this.$http.post('report/reportFile', value, {
            ...
            this.buttonVerifyVisible(this.reportFile !== null && this.reportFile.hasFile)

            const viewerOptions = {
              reportUrl: ko.observable(JSON.stringify({ type: 'report', user: this.$store.state.user.name, param: JSON.stringify(value) })),
              requestOptions: {
