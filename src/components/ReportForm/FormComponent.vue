<template>
  <div class="component-detail">
    <preserver-component v-bind="{ loading, disabled: !hasChanges || loading, saving }" @saveClick="onSaveClick()">
...
export default {
  ...
  methods: {
    ...
    async edit(id) {
      try {
        const { data } = await this.$http.get(`${this.formType === 'DbEquip' ? 'equip' : 'point'}Report/report`, { params: { id } })

        this.reportForm = new ReportForm(Object.assign(data, { type: this.formType }))
      } catch (error) {
