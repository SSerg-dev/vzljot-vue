<template>
  <div>
    <div class="exp-header">
      {{ caption }}
      <div v-if="resizable" :class="['exp-header-button', { close: localOpened }, 'fas', 'fa-angle-double-down']" @click="onClick()" :title="style.maxHeight === '0px' ? 'Показать' : 'Скрыть'"/>
    </div>
  ...
  export default {
    ...
    async mounted () {
      ...
      if (this.resizable) {
        ...
        this.style.maxHeight = (this.localOpened ? (slot ? slot.scrollHeight : 0) : 0) + 'px'
      }
