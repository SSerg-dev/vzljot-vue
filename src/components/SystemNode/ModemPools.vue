  ...
  <div class="component-detail">
    ...
    <ul class="root">
      <li v-for="(r, index) in localItems" :key="index">
        <div class="row" :style="{ 'grid-template-columns': `repeat(${$store.state.user?.userRights.modemPoolEdit ? 5 : 4}, max-content)` }">
          <input v-if="$store.state.user?.userRights.modemPoolEdit" type="checkbox" v-model="r.checked" />
          <span :class="getImage(r)" title="Просмотр..." @click="viewClick(r)" />
          <span>{{ r.name }}</span>
          <span :class="isModem(r) ? { warning: !serialPorts.some(port => port.number === r.comPort) } : null">{{
            isModem(r) ? portName(r.comPort) : ''
          }}</span>
          <span>{{ isModem(r) ? (r.comSpeeds ? r.comSpeeds.sort((a, b) => a - b).join('; ') : 'Все скорости') : '' }}</span>
        </div>
        <ul class="child" v-if="!isModem(r) && r.modems.length > 0">
          <li v-for="(modem, modemIndex) in r.modems" :key="modemIndex">
            <div class="row" :style="{ 'grid-template-columns': `repeat(${$store.state.user?.userRights.modemPoolEdit ? 5 : 4}, max-content)` }">
              <input v-if="$store.state.user?.userRights.modemPoolEdit" type="checkbox" v-model="modem.checked" />
              <span :class="getImage(modem)" title="Просмотр..." @click="viewClick(modem)" />
              <span>{{ modem.name }}</span>
              <span :class="{ warning: !serialPorts.some(r => r.number === modem.comPort) }">{{ portName(modem.comPort) }}</span>
              <span>{{ modem.comSpeeds ? modem.comSpeeds.sort((a, b) => a - b).join('; ') : 'Все скорости' }}</span>
            </div>
          </li>
        </ul>
      </li>
    ...
    <transition-group>
      <props-component v-if="component" v-bind="{ uuid: component.uuid, image: component.image, text: component.caption }" :key="0">
        <component :is="component.name" v-bind="component.data" @changed="onChanged" @saved="onSaved" />
      </props-component>
...
<script lang="ts">
import { computed, defineComponent, inject, onBeforeUnmount, PropType, provide, reactive, ref, Ref, watch } from 'vue'
...
function sortByName(a?: ISortable, b?: ISortable) {
  return store.state.collator.compare(a?.name.toLowerCase(), b?.name.toLowerCase())
}
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const localItems = ref([...provider.pools.map(r => new ModemPool(r)).sort(sortByName), ...provider.modems.map(r => new Modem(r)).sort(sortByName)])
    ...
    watch(
      ...
      value => {
        ...
        localItems.value = [...provider.pools.map(r => new ModemPool(r)).sort(sortByName), ...value.map(r => new Modem(r)).sort(sortByName)]
      },
      { deep: true }
    )
    ...
    watch(
      ...
      value => {
        ...
        localItems.value = [...value.map(r => new ModemPool(r)).sort(sortByName), ...provider.modems.map(r => new Modem(r)).sort(sortByName)]
      },
      { deep: true }
    )
          ...
          next(data: string) {
            if (data === 'modem') {
                ...
                next(data: string) {
                  if (data === 'none') {
                    ...
                    return {
                      ...
                      data: { modem: new Modem({}), error: new ModemError({}), modems: props.modems },
                      event: 'changed'
                    }
                  } else if (data === 'pool') {
                    return {
                      ...
                      next(data: Array<ModemPool>) {
                        ...
                        return {
                          text: `Создание модема:`,
                          component: 'modemProps',
                          data: { modem: new Modem({}), error: new ModemError({}), modems: selectedPool.modems },
                          event: 'changed'
                        }
                      }
                    }
              ...
              return {
                ...
                data: { modemPool: new ModemPool({}), error: new ModemPoolError({}) },
                event: 'changed'
              }
            }
          }
    ...
    const viewClick = (item: Modem | ModemPool) => {
      if (item instanceof Modem) {
        const pool = props.pools.find(r => r.modems.some(modem => modem.uuid === item.uuid))
    ...
    function onBeforeClose(event: { uuid: string }) {
      if (component.value && component.value.editable && component.value.uuid === event.uuid && component.value.hasChanges) {
        wizard.value = {
          name: 'save',
          component: {
            text: 'Сохранение:',
