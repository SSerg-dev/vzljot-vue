    ...
    <tabs>
      <tabx v-if="$store.state.user?.userRights.systemNode" text="Параметры">
        <preserver-component v-bind="{ readOnly, saving, disabled: !hasChanges || loading, loading }" @saveClick="save()">
          <system-node-props v-bind="{ systemNode: localItem, error: localError }" @changed="onChanged" @groupChanged="onGroupChanged" />
        </preserver-component>
      </tabx>
      <tabx v-if="$store.state.user?.userRights.systemNode" text="SIP-аккаунты">
        <preserver-component
          v-bind="{ readOnly, saving, disabled: !hasChanges || loading, loading }"
          @saveClick="save()"
        >
          <sip-accounts
      ...
      <tabx v-if="$store.state.user?.userRights.systemNode" text="Модемы и пулы">
        <preserver-component
          v-bind="{ readOnly, saving, disabled: !hasChanges || loading, loading }"
          @saveClick="save()"
        >
      ...
      <tabx v-if="$store.state.user?.userRights.systemNode && $store.state.user?.userRights.allowRedirectEndpoints" text="Перенаправление адаптеров">
        <preserver-component
          v-bind="{ readOnly, saving, disabled: !hasChanges || loading, loading }"
          @saveClick="save()"
        >
          <redirect-endpoints
            v-bind="{ items: localItem.endPoints, redirectEndPointProperties: localItem.redirectEndPointProperties }"
...
import { SystemNode, Group, Port, SystemNodeError } from '../../classes/systemNode'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const onSipAccountChanged = (sipAccount: SipAccount) => {
      const sip = localItem.value.sipAccounts.find(r => r.uuid === sipAccount.uuid)

      if (sip) {
        (Object.keys(sip) as (keyof ISipAccount)[]).forEach(r => ((sip[r] as string | number | boolean | undefined) = sipAccount[r]))
        hasChanges.value = true
      }
    }
    ...
    const onSipAccountRemove = (sipAccounts: Array<string>) => {
      localItem.value.sipAccounts = localItem.value.sipAccounts.filter(r => !sipAccounts.includes(r.uuid))
      hasChanges.value = true
    }
    ...
    function onPoolRemove(items: Array<string>) {
      localItem.value.modems = localItem.value.modems.filter(r => !items.includes(r.uuid))
      localItem.value.pools = localItem.value.pools.filter(r => !items.includes(r.uuid))
    ...
    const onModemChanged = (modem: Modem) => {
      ...
      localItem.value.pools.map(pool => pool.modems).forEach(r => modems.push(...r))
      modems.push(...localItem.value.modems)

      const local = modems.find(r => r.uuid === modem.uuid)

      if (local) {
        (Object.keys(local) as (keyof IModem)[]).forEach(r => ((local[r] as string | number | boolean | undefined | Array<number>) = modem[r]))
        hasChanges.value = true
      }
    }
    ...
    const onPoolChanged = (pool: ModemPool) => {
      ...
      if (local) {
        (Object.keys(local) as (keyof IModemPool)[]).forEach(r => ((local[r] as string | number | boolean | undefined | Array<Modem>) = pool[r]))
        hasChanges.value = true
      }
    }

    const onEndPointChanged = (redirectEndPoint: IRedirectEndPoint) => {
      const endPoint = localItem.value.endPoints.find(r => r.uuid === redirectEndPoint.uuid)

      if (endPoint) {
        (Object.keys(endPoint) as (keyof IRedirectEndPoint)[]).forEach(
          r => ((endPoint[r] as string | number | boolean | undefined) = redirectEndPoint[r])
        )
        hasChanges.value = true
      }
    }

    function onEndPointPropertiesChanged(redirectEndPointProperties: RedirectEndPointProperties) {
      localItem.value.redirectEndPointProperties = new RedirectEndPointProperties(redirectEndPointProperties)
      hasChanges.value = true
    }
    ...
    const onLoraServerChanged = (loraServer: ILoraServer) => {
      const server = localItem.value.loraServers.find(r => r.uuid === loraServer.uuid) as ILoraServer

      if (server) {
        (Object.keys(server) as (keyof ILoraServer)[]).forEach(r => ((server[r] as string | number | boolean | undefined) = loraServer[r]))
