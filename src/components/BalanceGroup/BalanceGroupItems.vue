<template>
  <div class="component-detail">
    ...
    <div class="table-grid" :style="`grid-template-columns: repeat(${$store.state.user?.userRights.balanceGroupEdit ? 4 : 3}, max-content)`">
      <header class="header" />
      <header class="header" v-if="$store.state.user?.userRights.balanceGroupEdit">
      ...
      <div v-for="(r, index) in localItems" :key="index" class="table-row">
        <span class="cell icon">
          <div :class="`${r.image} ${$store.state.env.statuses[r.state].class} table-icon`" />
        </span>
        <span class="cell" v-if="$store.state.user?.userRights.balanceGroupEdit">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const all = ref(false)
    const dataItems = ref<Array<BalanceGroup | Node | Point | PointList>>(props.items.map(create))
    ...
    const filteredItems = computed(() => {
      ...
      if (name) {
        rows = rows.filter(r => r.name.toLowerCase().includes(name.toLowerCase()))
      }
    ...
    function getType(r: BalanceGroup | Node | Point | PointList) {
      if (r instanceof BalanceGroup) {
        return `Балансовая группа${
          r.subGroup === BalanceSubGroupEnum.Sources ? ' (Источники)' : r.subGroup === BalanceSubGroupEnum.Consumers ? ' (Потребители)' : ''
        }`
      } else if (r instanceof Node) {
