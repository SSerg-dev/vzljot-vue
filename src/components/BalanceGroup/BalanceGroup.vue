  ...
  <div class="component-detail">
    <tabs>
      <tabx v-if="$store.state.user?.userRights.balanceGroup" text="Параметры">
        <preserver-component
          v-bind="{ readOnly: !$store.state.user?.userRights.balanceGroupEdit, saving, disabled: !hasChanges || loading, loading }"
          @saveClick="save()"
        >
          <balance-group-detail v-bind="{ name: localItem.name, error: localError }" @change="onChanged" />
        </preserver-component>
      </tabx>
      ...
      <tabx v-if="$store.state.user?.userRights.balanceGroup" text="Потребители">
        <preserver-component
          v-bind="{ readOnly: !$store.state.user?.userRights.balanceGroupEdit, saving, disabled: !hasChanges || loading, loading }"
          @saveClick="save()"
        >
      ...
      <tab text="Отчеты" v-if="$store.state.user?.userRights.report && reportData.reportTypes.length > 0">
