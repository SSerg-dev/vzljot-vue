<template>
  <div class="component-detail">
    <tool-bar>
      <div :class="['button', 'fas', 'fa-times-circle', { disabled: !hasSelected || wait }]" title="Удалить" @click="onDeleteClick" />
      <template v-slot:end>
    ...
    <div class="table-grid" :style="{ 'grid-template-columns': `repeat(${columnCount}, max-content)` }">
      <header class="header" />
      <header class="header" v-if="$store.state.user?.userRights.reportFileEdit"><input type="checkbox" v-model="all" @change="changeAll" :disabled="wait" /></header>
      <header class="header" />
      ...
      <header class="header" v-if="$store.state.user?.userRights.allowCryptSign">Подписано</header>
      <div v-for="(r, i) in localItems" :key="i" class="table-row">
        <span class="cell icon">
          <div :class="'fas fa-paperclip clickable-icon'" @click="viewClick(r)" title="Просмотр..." />
        </span>
        <span class="cell" v-if="$store.state.user?.userRights.reportFileEdit">
          <input type="checkbox" v-model="r.checked" :disabled="wait" />
        </span>
        <span class="cell icon">
          <span :class="`${getImage(r)} ${r.hasOwnProperty('state') ? $store.state.env.statuses[r.state].class : ''} table-icon`" />
        </span>
        <span class="cell">{{ r.objectName }}</span>
        <span class="cell">{{ r.reportName }}</span>
        <span class="cell">{{ r.taskName }}</span>
        <span class="cell">{{ $store.state.env.reportDataSource[r.source].text }}</span>
        <span class="cell">{{ new Date(r.timeStart).toLocaleString() }}</span>
        <span class="cell">{{ new Date(r.timeEnd).toLocaleString() }}</span>
        <span class="cell" v-if="$store.state.user?.userRights.allowCryptSign">{{ r.subject }}</span>
      </div>
    </div>
    <pager-component v-bind="pageInfo" @go="onChangePage" />
    <spinner :show="wait" :text="spinnerText" />
    <transition-group>
      <div v-if="reportDialog" style="position: absolute; top: 0; bottom: 0; left: 0; right: 0; background: rgba(0, 0, 0, 0.5); display: flex; flex-direction: column; overflow: auto; padding: 15px; transition: opacity 0.3s ease; z-index: 10">
        <div style="position: relative; display: flex; flex: 1">
          <div ref="viewer" style="background: white; display: flex; overflow: auto; flex: 1; flex-direction: column; width: 100%" data-bind="dxReportViewer: $data" />
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async onSignClick() {
      this.wizard = await wizardCertificates(this.$http, this.$store.state.user.id)
    },
    cancelWizard() {
      this.wizard = null
    },
    ...
    onCustomizeLocalization(s) {
      s.UpdateLocalization({
        ...
        'The document does not contain any pages.': 'Отчет не содержит ни одной страницы.',
        'The document is printed.': 'Отчет отправлен на печать.',
        'If nothing prints, or the printout is incorrect,': 'Если ничего не напечаталось,',
    ...
    async viewClick(reportFile) {
      ...
      try {
        ...
        const viewerOptions = {
          reportUrl: ko.observable(JSON.stringify({ type: 'file', user: this.$store.state.user.name, param: JSON.stringify({ id: reportFile.id }) })),
          requestOptions: {
    ...
    async remove(ids) {
      ...
      try {
        const { data } = await this.$http.delete('reportFile/reportFiles', { params: { ids } })
    ...
    async sign(id, certificate) {
      ...
      try {
        if (certificate.isLocal) {
          let res = await this.$http({ url: 'reportFile/reportPdf', data: { id }, method: 'POST', responseType: 'blob' })

          const blob = new Blob([res.data], {
            type: res.headers['content-type']
          })

          const signedData = await sign(certificate.subject, arrayBufferToBase64(await blob.arrayBuffer()))
