<template>
  <div class="chart-wrapper">
    <canvas :width="size.width" :height="size.height" ref="canvas" style="position: absolute;visibility:hidden;"></canvas>
    <div class="legend-wrapper" :style="legendStyle" v-if="localCharts.length > 0">
      <div :class="['legend-btn', 'fas', { 'fa-chevron-right': legendVisible }, { 'fa-chevron-left': !legendVisible }]"
        :style="$store.getters.styleColors" @click="legendVisible = !legendVisible"></div>
      <div style="overflow: auto;margin-right: 10px">
        <table class="legend-data">
          ...
          <tr v-for="(chart, index) in localCharts" :key="index">
            <td>
              <check-box :color="chart.color" v-model="chart.visible" @update:modelValue="updateCharts()"></check-box>
            </td>
            <td style="text-align: left;">{{ chart.text }}</td>
            <td>{{ chart.values.length === 0 ? 'нет' : Math.round(chart.minY * 1000) / 1000 }}</td>
            <td>{{ chart.values.length === 0 ? 'нет' : Math.round(chart.maxY * 1000) / 1000 }}</td>
            <td>{{ (chart.values.length > 0 && chart.values[line.index] && chart.values[line.index].y !== null) ? Math.round(chart.values[line.index].y * 1000) / 1000 : 'нет' }}</td>
          </tr>
        </table>
      </div>
    </div>
    <svg xmlns="http://www.w3.org/2000/svg" ref="svg" :viewBox="view" :width="size.width" :height="size.height" @mousemove="mousemove" @mouseout="mouseout" shape-rendering="geometricPrecision" >
      <text ref="y" visibility="hidden" dy=".32em">{{ axis.y.longestText }}</text>
      <text ref="x" visibility="hidden" dy=".32em">{{ axis.x.ticks ? axis.x.ticks.map(r => format(r)).toString() : ''}}</text>
      <g :transform="`translate(${padding.left},${padding.top})`">
        <g :transform="`translate(0,${chartHeight})`">
          <line style="pointer-events: none;" :x2="chartWidth" stroke="#333" />
          <g v-for="(value, index) in axis.x.ticks" :transform="`translate(${(x.way === 'reversed' ? axis.x.max - value : value - axis.x.min) * xRate},0)`"
            :key="index">
            <line style="pointer-events: none;" y2="6" stroke="#333" />
            <text dy=".71em" y="9" style="text-anchor: middle;font-size: 11px;">
              {{ format(value) }}
            </text>
            <line style="pointer-events: none;" :y2="-chartHeight" stroke="#eee" />
          </g>
        </g>
        <g>
          <line style="pointer-events: none;" :y2="chartHeight" stroke="#333" />
          <g v-for="(value, index) in axis.y.ticks" :transform="getYTickTransform(index)" :key="index">
            <line style="pointer-events: none;" x2="-6" stroke="#333" />
            <text dy=".36em" x="-8" style="text-anchor: end; font-size: 11px;">{{ value }}</text>
            <line v-if="index !== axis.y.ticks.length - 1" style="pointer-events: none;" :x2="chartWidth" stroke="#eee" />
          </g>
        </g>
        <g v-for="(chart, index) in localCharts" :key="index" :visibility="chart.visible ? 'visible' : 'hidden'">
          <path v-for="(path, index) in chart.path" :style="chart.type === 'point' ? pathPointStyle : pathStyle" :d="path"
            :stroke="chart.color" :key="index" />
        </g>
        <g>
          <g :visibility="line.visible ? 'visible' : 'hidden'">
            <line :y2="chartHeight" :x1="line.x" :x2="line.x" style="stroke-width: 1;pointer-events: none;" stroke="#333" />
            <text dy=".71em" :y="-11" :x="line.x" style="text-anchor: middle;">{{ format(xValues[line.index]) }}</text>
            <template v-for="(chart, index) in localCharts">
              <circle v-if="chart.visible && chart.values.length > 0 && chart.values[line.index] && chart.values[line.index].y" :key="index" :r="radius" :cx="getX(chart.values[line.index].x)"
                :cy="getY(chart.values[line.index].y)" :fill="chart.lightColor" :stroke="chart.color"
                @mouseover="circleOver" @mouseout="circleOut"
                style="transition: r 0.3s linear, stroke-width .3s linear;">
              </circle>
            </template>
          </g>
        </g>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      xValues() {
        return [...new Set(...Object.values(this.charts).map(r => r.values.map(x => x.x)))]
      },
      xRate() {
    ...
    methods: {
      ...
      updateCharts() {
        ...
        this.buildY(charts.length > 0 ? Math.min(...charts.map(r => r.minY)) : 0, charts.length > 0 ? Math.max(...charts.map(r => r.maxY)) : 0)

        this.$nextTick().then(() => {
          this.padding.left = Math.round((this.$refs.y.getBBox().width + 8) * 1000) / 1000
      ...
      buildCharts() {
        let charts = Object.keys(this.charts).map(key => {
          ...
          let ys = this.charts[key].values.filter(r => r.y !== null).map(r => r.y)
          return {
            name: key,
        ...
        this.buildX(charts.length > 0 ? Math.min(...charts.map(r => r.minX)) : 0, charts.length > 0 ? Math.max(...charts.map(r => r.maxX)) : 0)
        this.buildY(charts.length > 0 ? Math.min(...charts.map(r => r.minY)) : 0, charts.length > 0 ? Math.max(...charts.map(r => r.maxY)) : 0)
      ...
      mousemove(event) {
        ...
        if (x >= 0 && x <= this.chartWidth && y >= 0 && y <= this.chartHeight) {
          ...
          let index = this.binarySearch(this.xValues, value, 0, this.xValues.length - 1)
      ...
      binarySearch(values, value, start, end) {
        ...
        if (end - 1 === start) return Math.abs(values[start] - value) > Math.abs(values[end] - value) ? end : start
        switch (this.x.way) {
          case 'reversed':
            if (value < values[middle]) return this.binarySearch(values, value, middle, end)
            if (value > values[middle]) return this.binarySearch(values, value, start, middle)
            break
          case 'direct':
            if (value > values[middle]) return this.binarySearch(values, value, middle, end)
            if (value < values[middle]) return this.binarySearch(values, value, start, middle)
      ...
      format(value) {
        ...
        if (this.x.type === 'date') {
          ...
          let date = new Date(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(), d.getUTCHours(), d.getUTCMinutes(), d.getUTCSeconds())
        ...
        for (var i = len; i--;) {
          ...
          if (type === 'line') {
            if (y === null) {
              ...
              path = `${path}${path === '' ? 'M' : ' L'} ${this.getX(x)} ${this.getY(y)}`
            }
          } else if (type === 'point') {
            if (y !== null) {
              path = `${path}${path === '' ? '' : ' '}M ${this.getX(x) - r} ${this.getY(y)} a ${r},${r} 0 1,0 ${2*r},0 a ${r},${r} 0 1,0 ${-2*r},0`
            }
          }
        }
      ...
      getY(value) {
        ...
        return height === 0 ? 0 : (this.axis.y.max - value) / height * this.chartHeight
      },
      getYTickTransform(index) {
        var denominator = this.axis.y.ticks.length > 1 ? this.axis.y.ticks.length - 1 : 1
      ...
      export(type, file) {
        switch (type) {
          case 'jpeg':
          case 'png':
            exportSvg(type, file, this.$refs.svg.cloneNode(true).outerHTML, this.$refs.canvas)
            break
          case 'svg':
            {
