<template>
  <div class="component-detail">
    <preserver-component
      v-bind="{ readOnly: !$store.state.user?.userRights.setEdit, saving, disabled: !hasChanges || loading, loading }"
      @saveClick="save()"
    >
      <tabs>
        <tab text="Параметры">
          <set-params v-bind="{ set: localItem }" @addParams="onAddParams" @removeParams="onRemoveParams" @busy="onBusy" />
        </tab>
        <tab v-if="localItem.type === SetTypes.EquipGroup" text="Приборы">
          <set-equips v-bind="{ id: localItem.id, items: localItem.equips }" @addEquips="onAddEquips" @removeEquips="onRemoveEquips" />
        </tab>
        <tab text="Настройки">
          <set-detail v-bind="{ set: localItem, error: localError }" @changed="onChanged" />
        </tab>
      </tabs>
    </preserver-component>
    <transition-group>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const { hasChanges, loading, localItem, localError, onChanged, onWizardCancel, onWizardEnd, save, saving, wizard } = setupComponent<
      Set,
      SetError
    >(new Set({ id: props.id, uuid: props.uuid, equipId: props.equipId, equipTypeId: props.equipTypeId }), new SetError({}), emit)

    function onAddParams(data: Array<SetParam>) {
      localItem.value.params.splice(localItem.value.params.length, 0, ...data.map(r => new SetParam(r)))
    }

    function onRemoveParams(params: Array<SetParam>) {
      const ids = params.map(r => r.id)
      localItem.value.params = localItem.value.params?.filter(r => !ids.includes(r.id))
    }

    function onAddEquips(equips: Array<{ id: number, type: number, text: string, state: number, connectionGroupType: number}>) {
    ...
    function onRemoveEquips(equips: Array<Equip>) {
      ...
      localItem.value.equips = localItem.value.equips.filter(r => !ids.includes(r.equipId))
      localItem.value.params = localItem.value.params.filter(r => r.equipId && !ids.includes(r.equipId))
    }
