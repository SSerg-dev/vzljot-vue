<template>
  <div class="component-detail">
    <tool-bar :exportTypes="['csv', 'xls', 'xlsx']" @export="exportTable($event)">
      <div :class="['button', 'fas', 'fa-arrow-alt-circle-right', { disabled: !canRead }]" title="Собрать данные" @click="onPollClick(set.equipId, false)" />
      <div :class="['button', 'fas', 'fa-play-circle', { disabled: !canRead }]" title="Собирать данные" @click="onPollClick(set.equipId, true)" />
      <div :class="['button', 'fas', 'fa-arrow-alt-circle-left', { disabled: !canWrite }]" title="Записать данные" @click="onWriteClick(set.equipId)" />
      ...
      <template v-if="$store.state.user?.userRights.setEdit">
        <div class="separator" />
        <div :class="['button', 'fas', 'fa-plus-circle', { disabled: busy }]" title="Добавить..." @click="onAddParamClick()" />
        <div :class="['button', 'fas', 'fa-times-circle', { disabled: !isAnyParam || busy }]" title="Удалить" @click="onRemoveParamClick()" />
      </template>
      <template v-slot:end>
        <div :class="['button', 'fas', 'fa-cog', { disabled: busy }]" title="Настройки" @click="onSettingsClick" />
      </template>
    </tool-bar>
    <div class="table-grid" style="grid-template-columns: repeat(9, max-content)">
      <header class="header"></header>
      <header class="header">Время получения</header>
      <header class="header"><input type="checkbox" v-model="all" @change="changeAll" :disabled="busy" /></header>
      ...
      <div v-for="(r, i) in localParams" :key="i" class="table-row" :ref="el => (rowsElement[r.id] = el)">
        <span class="cell icon">
          <div :class="r.task.class" :style="{ color: r.task.style.color }" :title="r.task.text" />
        </span>
        <span class="cell cell-value" style="justify-content: center">{{ r.time }}</span>
        <span class="cell">
          <input type="checkbox" v-model="r.checked" :disabled="busy" />
        </span>
        <span class="cell">{{ r.equipTypeName }}</span>
        <span class="cell" @click="channelClick($event.target, localParams[i])">{{ r.channelNumber ? r.channelNumber : '?' }}</span>
        <span class="cell cell-value" @click="valueClick($event.target, localParams[i])">{{ r.value }}</span>
        <span class="cell">{{ r.name }}</span>
        <span class="cell" @click="muClick($event.target, localParams[i])">{{ $store.state.env.mUnits[r.mu].text }}</span>
        <span class="cell" @click="dataTypeClick($event.target, localParams[i])">{{ r.dataType }}</span>
      </div>
    </div>
    <pager-component v-bind="pageInfo" @go="onChangePage" />
    <spinner :show="wait" :text="'Загрузка данных...'" />
    <select ref="muSelect" v-show="muPossible" v-model="mu" @blur=";(mu = null), (muGroup = null)" @change="onMuChange" v-on:click.stop style="position: absolute">
      <option v-for="r in muList" :key="r.value" :value="r.value">{{ r.text }}</option>
    </select>
    <select ref="dataTypeSelect" v-show="dataTypePossible" v-model="dataType" @blur="dataType = null" @change="onDataTypeChange" v-on:click.stop style="position: absolute">
      <option v-for="r in dataTypes" :key="r" :value="r">{{ r }}</option>
    </select>
    <input type="text" ref="valueInput" v-show="valueVisible && valuePossible" v-model="value" @blur="valueVisible = false" @change="onValueChange" style="position: absolute" />
    <input
      type="text"
  ...
  return {
    ...
    component: {
      ...
      data: {
        singleMode: true,
        loader: async () => {
          let { data } = await http.get('set/getSelectorEquips', { params: { equipTypeId } })
          return data
        }
      }
    }
...
let keyCodes = new Array(...'0123456789 ,-'.split(''), 'Backspace', 'Delete', 'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Space', 'Tab', 'Enter')
...
export default {
  ...
  computed: {
    canWrite() {
      return !this.busy && this.isAnyParam && this.$store.state.user?.userRights.dataWrite
    },
    canRead() {
      return !this.busy && this.isAnyParam && this.$store.state.user?.userRights.dataRead
    },
    isAnyParam() {
      return this.set.params.some(r => r.checked === true)
    },
    muPossible() {
      return this.mu && (this.muGroup ? this.muGroup.units.length > 1 : false) && this.convert && !this.busy
    },
    valuePossible() {
      return !this.busy
    },
    ...
    localParams() {
      ...
      const lastIndex = this.pageInfo.Page * this.pageInfo.Size > this.set.params.length ? this.set.params.length : this.pageInfo.Page * this.pageInfo.Size
  ...
  methods: {
    ...
    updateChannelNumber() {
      const minValue = this.$store.state.env.equipTypeCodes[this.clickedRow.equipTypeId].type !== 'modbus' ? 1 : 0
      const maxValue = this.$store.state.env.equipTypeCodes[this.clickedRow.equipTypeId].type !== 'modbus' ? this.clickedRow.channelNumber >> 16 : 65535
      ...
      let rawValues = this.channelNumber.toString().replace(/\s+/g, '').split(',')
      let result = []
    ...
    addChannelNumber(param, values) {
      ...
      param.name = this.updateNameByChannel(param.equipTypeParamName, param.name, param.channelNumber)

      if (this.$store.state.env.equipTypeCodes[param.equipTypeId].type === 'modbus') {
      ...
      this.$emit(
        ...
        values.map(value => {
          ...
          r.name = this.updateNameByChannel(r.equipTypeParamName, r.name, r.channelNumber)
    ...
    onPollClick(equipId, loop) {
      if (this.set.type === this.setTypes.equipType) {
        if (equipId) {
          ...
          this.wizard = wizardEquip(this.set.equipTypeId, loop ? 'equipLoop' : 'equip')
        }
      } else if (this.set.type === this.setTypes.equipGroup) {
    ...
    muClick(cell, row) {
      ...
      this.mu = row.mu
      if (row.mu) {
        this.muGroup = this.$store.state.env.mUnitGroups[this.$store.state.env.mUnits[this.mu].group]
    ...
    channelClick(cell, row) {
      ...
      this.clickedRow = row
      if (this.$store.state.env.equipTypeCodes[row.equipTypeId].type === 'modbus' || this.$store.state.env.equipTypeCodes[row.equipTypeId].type === 'SPE542') {
        this.setPosition(cell, this.$refs.channelNumberInput)
        this.channelNumber = !isNaN(parseFloat(row.channelNumber)) && isFinite(row.channelNumber) ? row.channelNumber : ''
    ...
    processMessage(message) {
      if ((message.data.action === messageAction.poll || message.data.action === messageAction.write) && message.data.key === this.key) {
        let rows = []

        if (message.data.hasOwnProperty('paramId')) {
          rows = this.set.params.filter(r => r.equipTypeParamId === message.data.paramId && (message.data.hasOwnProperty('channelNumber') ? r.channelNumber === message.data.channelNumber : true))
        } else if (message.data.hasOwnProperty('equips')) {
          rows = this.set.params.filter(r => (message.data.equips.includes(r.equipId) || r.equipId === undefined) && r.checked)
        }
        ...
        rows.forEach(r => {
          ...
          if (message.data.state === 'success') {
            ...
            if (r.dataType === 'dateTime' && typeof message.data.value === 'string' && matcher.isoDate.test(message.data.value)) {
              message.data.value = new Date(message.data.value)
            }

            r.value = message.data.value instanceof Date ? this.formatDate(message.data.value) : message.data.value
          ...
          if (this.rowsElement[r.id]) {
            ...
            setTimeout(removeClass, 500, this.rowsElement[r.id], 'param-animation')
          }
        })
      }
    },
      ...
      if (this.set.params.some(r => r.checked)) {
        if (this.set.params.some(r => r.equipMu === undefined && r.checked)) {
          ...
          const parameters = this.set.params
            ...
            .map(r => {
              return {
                equipId: this.set.type === this.setTypes.equipType ? equipId : r.equipId,
