<template>
  <div class="component-detail">
    ...
    <div class="table-grid" style="grid-template-columns: repeat(6, max-content)">
      ...
      <div v-for="(r, index) in dataItems" :key="index" class="table-row">
        <span class="cell icon">
          <div :class="`${getImage(r)} clickable-icon`" title="Просмотр..." @click="viewClick(r)" />
        </span>
        <span class="cell">{{ r.text }}</span>
        <span class="cell">{{ $store.state.env.setTypes[r.setType].text }}</span>
...
export default {
  ...
  methods: {
    ...
    sort(a, b) {
      return this.$store.state.collator.compare(a.text.toLowerCase(), b.text.toLowerCase())
    },
    onSaved(data) {
