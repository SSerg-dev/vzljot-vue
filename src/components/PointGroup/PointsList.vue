<template>
  <div class="component-detail">
    ...
    <div
      class="table-grid"
      :style="{ 'grid-template-columns': `repeat(${$store.state.user?.userRights.measureSchemeGroupEdit ? '5' : '4'}, max-content)` }"
    >
      ...
      <div v-for="(r, index) in localItems" :key="index" class="table-row">
        <span class="cell" v-if="$store.state.user?.userRights.measureSchemeGroupEdit">
          <input type="checkbox" v-model="r.checked" />
        </span>
        <span class="cell">{{ dataItems.indexOf(r) + 1 }}</span>
        <span class="cell icon">
          <span :class="`${r.image} ${$store.state.env.statuses[r.state].class} table-icon`" />
        </span>
        <span class="cell icon">
          <span :class="`${r.getSystemImage()} table-icon`" />
        </span>
