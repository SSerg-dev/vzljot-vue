<template>
  <div class="component-detail">
    <toolbar :exportTypes="viewType === 'table' ? ['csv', 'xls', 'xlsx'] : ['jpeg', 'png', 'svg']" @export="exportData(id, localTimeStart.getTime(), localTimeEnd.getTime(), $event)">
      <label
        >с:
        <date-picker v-model="localTimeStart" :format="dateFormat" style="width: 90px;" />
      </label>
      <label
        >по:
        <date-picker v-model="localTimeEnd" :format="dateFormat" style="width: 90px;" />
      </label>
      <template v-slot:end>
    ...
    <tabs @click="viewType = $event">
      ...
      <tab text="График по tнв" name="chartnv">
        <chart-line ref="chartnv" v-bind="{ x: { way: 'reversed', type: 'value' }, charts: this.chartsByTnv }" />
      </tab>
      <tab text="Данные" name="table">
        <div class="table-grid" :style="{ 'grid-template-columns': `repeat(${columns.length}, max-content)` }">
          <header v-for="(r, i) in columns" :key="i" class="header right">{{ r.name }}</header>
          <div v-for="(r, i) in data" :key="i" class="table-row">
            ...
            <span :class="['cell', { right: true, 'warn-color': r.t1Event }]">{{ r.dt1 }}</span>
            <span :class="['cell', { right: true, 'warn-color': r.t1Event }]">{{ r.dt1Per }}</span>
            <span :class="['cell', { right: true, 'warn-color': r.t1Event }]">{{ r.t1Event === true ? 'Да' : r.T1Event === false ? 'Нет' : '' }}</span>
            <span class="cell right">{{ r.t2 }}</span>
            <span class="cell right">{{ r.t2gr }}</span>
            <span :class="['cell', { right: true, 'warn-color': r.t2Event }]">{{ r.dt2 }}</span>
            <span :class="['cell', { right: true, 'warn-color': r.t2Event }]">{{ r.dt2Per }}</span>
            <span :class="['cell', { right: true, 'warn-color': r.t2Event }]">{{ r.t2Event === true ? 'Да' : r.T2Event === false ? 'Нет' : '' }}</span>
          </div>
        </div>
    ...
    async exportData(id, timeStart, timeEnd, type) {
      ...
      switch (type) {
        ...
        default:
          this.wait = true
          try {
            await this.exporter.post('analyze/export', { id, timeStart, timeEnd, type, analyzeType: 'temperature' })
          } catch (error) {
            this.$store.commit('error', error)
          } finally {
            this.wait = false
