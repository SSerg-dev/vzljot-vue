<template>
  <div class="component-detail">
    <tool-bar>
      <div v-if="$store.state.user?.userRights.systemMessageStatusReset" :class="['button', 'fas', 'fa-times-circle', { disabled: !hasSelected || wait }]" title="Очистить" @click="onRemoveClick" />
      <template v-slot:end>
    ...
    <div class="table-grid" :style="{ 'grid-template-columns': `repeat(${columnsCount}, max-content)` }">
      ...
      <header v-if="$store.state.user?.userRights.systemMessageStatusReset"><input type="checkbox" v-model="all" @change="changeAll" :disabled="wait" /></header>
      ...
      <div v-for="(r, i) in localItems" :key="i" class="table-row">
        ...
        <span class="cell icon" v-if="$store.state.user?.userRights.systemMessageStatusReset">
          <input v-if="r.reset" type="checkbox" v-model="r.checked" :disabled="wait" />
        </span>
        <span class="cell">{{ r.recieve ? new Date(r.recieve).toLocaleString() : '' }}</span>
        <span class="cell">{{ r.type }}</span>
        <span class="cell">{{ r.text }}</span>
        <span class="cell">{{ r.message }}</span>
        <span class="cell">{{ r.event ? new Date(r.event).toLocaleString() : '' }}</span>
        <span class="cell">{{ r.lastEvent ? new Date(r.lastEvent).toLocaleString() : '' }}</span>
    ...
    <transition-group>
      ...
      <props-component v-if="edit" :text="`Анализ: ${component.text}`" @close="close">
...
export default {
  ...
  computed: {
    ...
    columnsCount() {
      ...
      if (!this.$store.state.user?.userRights.systemMessageStatusReset) columns--
      return columns
    }
  },
  watch: {
  ...
  methods: {
    ...
    async viewClick(r) {
      this.wait = true
      try {
        const { data } = await this.$http.get('systemMessage/status', { params: { id: r.id, objectId: r.objectId } })

        this.component = {
          name: data.component,
          data: {
            id: data.id,
            timeStart: r.timeEvent ? new Date(r.timeEvent) : new Date(new Date().getFullYear(), new Date().getMonth() - 1, new Date().getDate()),
    ...
    async remove(ids) {
      this.wait = true
      try {
        const { data } = await this.$http.delete('systemMessage/states', { params: { ids } })

        if (Object.prototype.hasOwnProperty.call(data, 'type') && this.$store.state.env.resultTypes[data.type].type === 'success') {
