<template>
  <div style="font-size: 14px;padding: 15px;min-width: 450px;">
    <div v-for="(r, i) in options" :key="i" style="padding: 3px;" >
      <radio :label="r.type" v-model="option" @change="handleChange">{{ r.text }}</radio>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        option: this.defaultOption ? this.defaultOption.type : (this.options ? (this.options.length > 0 ? this.options[0].type : null) : null)
      }
    },
    mounted() {
      this.$nextTick().then(() => {
