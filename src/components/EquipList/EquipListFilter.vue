<template>
  <div class="equip-list-filter">
    ...
    <select v-model="localFilter.state" @change="onChange">
      <option v-for="[k, v] in localStatuses" :key="k" :value="k">{{ v }}</option>
    </select>
    <label>Прибор:</label>
    ...
    <select v-model="localFilter.column" @change="onChange">
      <option v-for="[k, v] in localColumnTypes" :key="k" :value="k">{{ v }}</option>
    </select>
    <label>с:</label>
    <div class="equip-list-filter">
      <date-picker v-model="localFilter.periodStart" format="DD.MM.YYYY" clearable style="width: 90px" @update:modelValue="onChange" />
      <div class="equip-list-filter">
        по:<date-picker v-model="localFilter.periodEnd" format="DD.MM.YYYY" clearable style="width: 90px" @update:modelValue="onChange" />
      </div>
    </div>
  ...
  setup(props, { emit }) {
    ...
    const localTypes = [
      ...
      ...props.types
        ...
        .sort((a, b) => {
          return collator.compare((a[1] as string).toLowerCase(), (b[1] as string).toLowerCase())
        })
    ]
