  ...
  <div class="component-detail">
    <tool-bar>
      <div
        ...
        :class="['button', 'fas', 'fa-arrow-alt-circle-right', { disabled: !hasSelected || polling }]"
      ...
      <template v-if="$store.state?.user.userRights.equipList">
        ...
        <div :class="['button', 'fas', 'fa-times-circle', { disabled: !hasSelected || polling }]" title="Удалить" @click="onRemoveClick()" />
      </template>
      <template v-slot:end>
        <div
        ...
        <div
          :class="[
            ...
            'fa-filter',
            'remove-sign',
            { disabled: !hasFilterChanges, right: !$store.state.user?.userRights.pollDataManual }
          ]"
    ...
    <div class="table-grid" :style="`grid-template-columns: repeat(${columnsCount}, max-content)`">
      <header class="header sticky-first column" :style="`grid-template-columns: repeat(${firstColumnsCount - 1}, min-content)`">
        ...
        <input v-if="$store.state.user?.userRights.equipList" type="checkbox" v-model="all" @change="changeAll" :disabled="polling" />
        Прибор
      </header>
      ...
      <div v-for="(r, index) in localItems" :key="index" class="table-row">
        <span class="cell sticky-first column" :style="`grid-template-columns: repeat(${firstColumnsCount}, min-content)`">
          ...
          <input
            ...
            :disabled="polling"
          />
          <span :class="`fas fa-tablet-alt ${$store.state.env.statuses[r.state].class} table-icon`" />{{ r.name }}
        </span>
        <span class="cell">
          <a v-for="node in r.nodes" :key="node.id" @click="loadNode(node.id)" href="#" :title="node.fullName">{{ node.name }}; </a>
        </span>
        <span class="cell">
          <a v-for="(point, indexPoint) in r.points" :key="indexPoint" @click="loadPoint(point.id)" href="#" :title="point.fullName"
            >{{ $store.state.env.pointTypes[point.systemType].text }};
          </a>
        </span>
        <span class="cell">{{ r.endPoint }}</span>
        <span class="cell">{{
          r.archiveHourTime ? `${r.archiveHourTime.toLocaleDateString()}, ${r.archiveHourTime.getHours().toString().padStart(2, '0')}` : ''
        }}</span>
        <span class="cell">{{ r.archiveDayTime ? r.archiveDayTime.toLocaleDateString() : '' }}</span>
        <span class="cell">{{
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const filteredItems = computed(() => {
      ...
      if (name) {
        rows = rows.filter(r => r.name.toLowerCase().indexOf(name.toLowerCase()) >= 0)
      }

      const nodeName = filter.value.equip
      if (nodeName) {
        rows = rows.filter(r => r.nodes.some(node => node.name.toLowerCase().indexOf(nodeName.toLowerCase()) >= 0))
      }

      const port = filter.value.port
      if (port) {
        rows = rows.filter(r => r.endPoint.toLowerCase().indexOf(port.toLowerCase()) >= 0)
      }

      if (filter.value.type) {
        rows = rows.filter(r => r.points.length > 0 && r.points.some(p => p.systemType === filter.value.type))
      }
      ...
      if (column) {
        ...
        if (filter.value.periodStart) {
          rows.filter(r => comparePeriodStart(filter.value.periodStart, r[column] as Date)).forEach(r => set.add(r))
        }
        if (filter.value.periodEnd) {
          rows.filter(r => comparePeriodEnd(filter.value.periodEnd, r[column] as Date)).forEach(r => set.add(r))
        }
        ...
        if (filter.value.periodStart) {
          Object.values(ColumnTypeEnum).forEach(column =>
            rows.filter(r => comparePeriodStart(filter.value.periodStart, r[column as keyof Equip] as Date)).forEach(r => set.add(r))
          )
        }

        if (filter.value.periodEnd) {
          Object.values(ColumnTypeEnum).forEach(column =>
            rows.filter(r => comparePeriodEnd(filter.value.periodEnd, r[column as keyof Equip] as Date)).forEach(r => set.add(r))
          )
        }
    ...
    const hasFilterChanges = computed(() => JSON.stringify(filter.value) !== JSON.stringify(new Filter({})))
    ...
    function comparePeriodStart(filterValue: Date, value: Date) {
      return value && value.getTime() >= new Date(filterValue.getFullYear(), filterValue.getMonth(), filterValue.getDate(), 0, 0, 0).getTime()
    }

    function comparePeriodEnd(filterValue: Date, value: Date) {
      return value && value.getTime() <= new Date(filterValue.getFullYear(), filterValue.getMonth(), filterValue.getDate(), 23, 59, 59).getTime()
    }
        ...
        component: {
          ...
          data: {
            loader: async () => {
              ...
              const { data } = await axios.get<Array<{ id: number; type: number; name: string; connectionGroupType: number; state: number }>>(
                'equipList/equips'
              )
          ...
          if (equip) {
            ...
            if (row && row.task) {
              if (message.data.state === 'success') {
                row.archiveHourTime = equip.archiveHourTime ? new Date(equip.archiveHourTime) : undefined
                row.archiveDayTime = equip.archiveDayTime ? new Date(equip.archiveDayTime) : undefined
                row.archiveMonthTime = equip.archiveMonthTime ? new Date(equip.archiveMonthTime) : undefined
                row.pollCurrentTime = equip.pollCurrentTime ? new Date(equip.pollCurrentTime) : undefined
    ...
    function onSi(message: SiMessage) {
      if (message && message.event === 'update' && store.state.env.itemTypes[message.type].type === 'equip') {
        dataItems.value.filter(r => r.equipId === message.item.id).forEach(r => (r.state = message.item.state))
      }
    }
    ...
    function onWizardEnd(data: any, name: string) {
      ...
      if (name === 'filter') {
        ...
        emit(
          'remove',
          dataItems.value.filter(r => r.checked === true).map(r => r.equipId)
        )
      }
    }
