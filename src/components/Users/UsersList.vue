<template>
  <div class="component-detail">
    ...
    <div class="table-grid" :style="{ 'grid-template-columns': `repeat(5, max-content)` }">
      ...
      <div v-for="(r, i) in localItems" :key="i" class="table-row">
        <span class="cell icon">
          <div :class="`${getImage(r)} clickable-icon`" style="position: relative" @click="viewClick(r)" title="Просмотр..." />
        </span>
        <span class="cell">
          <input v-if="r.name !== $store.state.env.adminName" type="checkbox" v-model="r.checked" :disabled="wait" />
        </span>
    ...
    <transition-group>
      ...
      <props v-if="edit" :text="`Пользователь: ${componentText}`" @close="close">
...
export default {
  ...
  computed: {
    filteredItems() {
      ...
      if (this.filter.groups !== null) {
        rows = rows.filter(r => r.groups.map(x => x.userGroupId).includes(this.filter.groups))
      }

      if (this.filter.desc !== null) {
        rows = rows.filter(r => (r.desc ? r.desc.toLowerCase().includes(this.filter.desc.toLowerCase()) : false))
      }
  ...
  methods: {
    ...
    onUpdate(item) {
      ...
      } else if (this.$store.state.env.itemTypes[item.type].type === 'userGroupItem') {
        const user = this.dataItems.find(r => r.id === item.userId)
        if (user) {
          if (user.groups.filter(r => r.id === item.id).length === 0) {
            user.groups.push({ id: item.id, userGroupId: item.userGroupId, name: item.name })
          }
        }
      }
    },
    onDelete(obj) {
      ...
      } else if (this.$store.state.env.itemTypes[obj.type].type === 'userGroupItem') {
        const user = this.dataItems.find(r => r.groups.map(x => x.id).includes(obj.id))
    ...
    processMessage(message) {
      if (message.data.action === 'onlineUsers') {
        this.dataItems.forEach(r => (r.online = message.data.users.length > 0 && message.data.users.includes(r.id)))
      }
    },
    async load() {
      try {
        ...
        data.forEach(r => {
          r.checked = r.name === this.$store.state.env.adminName ? false : this.all
          r.online = false
        })
        this.dataItems = data
    ...
    async remove(ids) {
      ...
      try {
        const { data } = await this.$http.delete('user/users', { params: { ids } })
