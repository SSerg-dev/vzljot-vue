<template>
  <div class="poll-data-settings-grid">
    <check-box v-model="value.useSystem" @update:modelValue="onChange()">Использовать параметры автоматического сбора данных</check-box>
    <div class="poll-data-settings-grid two">
      <label :disabled="disabled">Данные:</label>
      <div style="display: grid; gap: 5px 3px; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr))">
        <template v-for="[k, v] in Object.entries($store.state.env.pollDataTypes)">
          <check-box v-if="value[v.type] !== null" :key="k" v-model="value[v.type]" :disabled="disabled" @update:modelValue="onChange()" :class="{ 'validation-error': error.data.value }" :title="error.data.title">{{ v.text }}</check-box>
        </template>
      </div>
    </div>
    <div class="poll-data-settings-grid two">
      ...
      <div class="poll-data-settings-grid two">
        <select :disabled="disabled" v-model="value.timeType" @change="onChange()">
        ...
        <div class="poll-data-settings-grid" style="min-width: 250px">
          <div v-if="value.timeType === pollDataTimeTypes.onDepth" class="poll-data-settings-grid two">
            <number-box :disabled="disabled" v-model="value.timeDepth" @update:modelValue="onChange()" style="width: 30px" :min="1" :max="999" />
          ...
          <div v-else style="display: grid; align-items: center; grid-gap: 5px 3px; grid-template-columns: auto auto auto 1fr">
            <date-picker :disabled="disabled" v-model="value.timeFrom" @update:modelValue="onChange()" format="DD.MM.YYYY" :class="{ 'validation-error': error.time.value }" :title="error.time.title" />
            <check-box :disabled="disabled" v-model="value.isTimeTo" @update:modelValue="onChange()" />
            <label :disabled="disabled || !value.isTimeTo">по</label>
            <date-picker :disabled="disabled || !value.isTimeTo" @update:modelValue="onChange()" v-model="value.timeTo" format="DD.MM.YYYY" :class="{ 'validation-error': error.time.value }" :title="error.time.title" />
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    validate() {
      ...
      if (this.value.timeType === this.pollDataTimeTypes.fromTime) {
        if (this.value.isTimeTo && new Date(this.value.timeFrom).getTime() > new Date(this.value.timeTo).getTime()) {
          this.error.time.value = true
          this.error.time.title = 'Дата окончания не может быть меньше даты начала.'
        }
      }
