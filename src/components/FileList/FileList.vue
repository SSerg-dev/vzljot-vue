  ...
  <div class="component-detail">
    <toolbar>
      <label v-if="userRight" style="margin: 0;">
        <input type="file" multiple style="display: none;" @change="handleFiles($event.target.files)" />
        <div :class="['button', 'fas', 'fa-plus-circle', { disabled: wait }]" title="Добавить..." />
      </label>
      <div v-if="userRight" :class="['button', 'fas', 'fa-times-circle', { disabled: !hasSelected || wait }]" title="Удалить" @click="onRemoveClick()" />
      <template v-slot:end>
    ...
    <div class="table-grid" :style="{ 'grid-template-columns': `repeat(${userRight ? '7' : '6'}, max-content)` }">
      <header class="header" />
      <header class="header" />
      ...
      <div v-for="(r, index) in localItems" :key="index" class="table-row">
        <span class="cell icon">
          <div :class="'fas fa-paperclip clickable-icon'" @click="viewClick(r.id, true)" title="Просмотр..." />
        </span>
        <span class="cell icon">
          <div :class="'fas fa-download clickable-icon'" @click="viewClick(r.id, false)" title="Загрузка..." />
        </span>
        <span class="cell" v-if="userRight"><input type="checkbox" v-model="r.checked" :disabled="wait"/></span>
        <span class="cell">{{ r.name }}</span>
        <span class="cell">{{ r.timeChanged ? new Date(r.timeChanged).toLocaleDateString() : '' }}</span>
...
export default {
  ...
  data() {
    return {
      ...
      all: false,
      wizard: null,
      fileReaderWorker: new Worker(new URL('../../plugins/workers/file.worker.js', import.meta.url)),
      exporter: new Export(this.$http)
    }
  },
  computed: {
    userRight() {
      if (this.type === 'DbMeasureScheme') return this.$store.state.user?.userRights.measureSchemeEdit
      if (this.type === 'DbEquip') return this.$store.state.user?.userRights.equipEdit
      if (this.type === 'DbNode') return this.$store.state.user?.userRights.measureSchemeEdit
      return false
    }
  },
  mounted() {
