...
<div :class="`${pre}`">
  ...
  <div :class="`${pre}-foot`" v-if="m==='H'">
    <div :class="`${pre}-hour`">
      <a :title="local.hourTip" @click="showHours=!showHours,showMinutes=showSeconds=false" :class="{on:showHours}">{{hour}}</a>
      <span>:</span>
      <a :title="local.minuteTip" @click="showMinutes=!showMinutes,showHours=showSeconds=false" :class="{on:showMinutes}">{{minute}}</a>
      <span>:</span>
      <a :title="local.secondTip" @click="showSeconds=!showSeconds,showHours=showMinutes=false" :class="{on:showSeconds}">{{second}}</a>
    </div>
  </div>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    status (year, month, day, hour, minute, second, format) {
      ...
      classObj[`${this.pre}-date-disabled`] = (this.right && t < this.start) || (this.left && t > this.end) || this.disabledDate(time)
    ...
    ok (info) {
      ...
      const date = new Date(year || this.year, month || this.month, this.day, this.hour, this.minute, this.second)
