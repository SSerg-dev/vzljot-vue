<template>
  <div class="vue-chart-pie-container">
    <div class="legend">
      <div v-for="(key, index) in Object.keys(localStatuses).sort()" :key="index" :ref="localStatuses[key].type" class="legend-item" @mouseover="legendOver(key)" @mouseout="legendOut(key)">
        <div :class="`mark fas fa-square icon ${localStatuses[key].class}`" />
        {{ localStatuses[key].text }}
      </div>
    </div>
    <svg :viewBox="view" class="vue-chart-pie">
      <circle v-if="pathes.length === 1" :r="radius" :cx="viewBox.width / 2" :cy="viewBox.height / 2" fill="none" :ref="pathes[0].key"
        :stroke="pathes[0].state.color" @mouseover="pathIn(pathes[0])" @mouseout="pathOut(pathes[0])"></circle>
      <path v-else v-for="(path, index) in pathes" :d="path.d" fill="none" :stroke="path.state.color" :ref="path.key" :key="index" @mouseover="pathIn(path)"
        @mouseout="pathOut(path)"></path>
      <text :x="viewBox.width / 2" :y="viewBox.height / 2" dy=".3em" style="text-anchor: middle;">
        {{ text }}
      </text>
    </svg>
  </div>
...
<script>
  export default {
    ...
    computed: {
      ...
      radius() {
        return Math.min(this.viewBox.width, this.viewBox.height) / 2 - this.strokeWidthSelected;
      }
    },
    methods: {
      ...
      legendOver(key) {
        if (this.pathes.length === 1) {
          if (this.pathes[0].key.toString() === key) {
            this.text = `${this.pathes[0].value} (${Math.round(this.pathes[0].percent * 100 * 100) / 100}%)`
            this.$refs[key].style.strokeWidth = this.strokeWidthSelected
          }
        } else {
          this.pathes.forEach(path => {
            if (path.key.toString() === key) {
              this.text = `${path.value} (${Math.round(path.percent * 100 * 100) / 100}%)`
