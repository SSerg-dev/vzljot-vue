<template>
  <div class="component-detail">
    <tool-bar :exportTypes="['csv', 'xls', 'xlsx']" @export="exportArchive">
      <label
        >с:
        <date-picker v-model="periodStart" :format="dateFormat" style="width: 150px" @update:modelValue="debounce()" />
      </label>
      <label
        >по:
        <date-picker v-model="periodEnd" :format="dateFormat" style="width: 150px" @update:modelValue="debounce()" />
      </label>
      <label
        >Тип архива:
        <select v-model="archiveType" @change="onChange()">
          <option v-for="(key, index) in Object.keys(archiveTypes)" :key="index" :value="parseInt(key)">{{ archiveTypes[key].text }}</option>
        </select>
      </label>
      <template v-slot:end>
        <div class="button fas fa-cog" title="Настройки" @click="onSettingsClick()" />
      </template>
    </tool-bar>
    <div v-if="localItems.length > 0" class="table-grid" :style="`grid-template-columns: repeat(${columns.length}, max-content)`">
      <div class="table-row">
        <header v-for="(r, i) in columns" :key="i" :class="['header', 'right', { 'sticky-first': i === 0 }, { active: i === sortIndex }]">
      ...
      <div class="table-row" v-for="(row, rowIndex) in localItems" :key="rowIndex">
        <span :class="['cell', 'right', { hint: hasHint(r) }, { 'sticky-first': i === 0 }]" v-for="(r, i) in row" :key="(rowIndex + 1) * (i + 1)" :title="getCellTitle(r)">{{ getCellValue(r) }}</span>
      </div>
    </div>
...
const wizardSettings = (hideEmptyFields, hideDeltaFields, hideTotalFields, measures) => {
  return {
    name: 'settings',
    component: {
      text: 'Настройки:',
  ...
  methods: {
    ...
    onChange() {
      if (this.archiveType === 3) {
        this.periodStart = new Date(this.periodStart.getFullYear(), this.periodStart.getMonth(), 1)
        this.periodEnd = new Date(this.periodEnd.getFullYear(), this.periodEnd.getMonth() + 1, 1, 0, 0, -1)
      }

      this.get(1, this.pageInfo.Size)
    },
    onSettingsClick() {
      this.wizard = wizardSettings(this.hideEmptyFields, this.hideDeltaFields, this.hideTotalFields, this.measures)
    },
    cancelWizard() {
      this.wizard = null
    },
    ...
    async exportArchive(exportType) {
      this.wait = true
      try {
        await this.exporter.post('archive/export', Object.assign(this.getParam(), { exportType }))
      } catch (error) {
    ...
    async get(page, size) {
      ...
      try {
        const { data } = await this.$http.post('archive/archive', Object.assign(this.getParam(), { pageInfo: { page, size } }))
    ...
    getParam() {
      ...
      if (Array.isArray(this.measures) && this.measures.length) {
        param.measures = this.measures.map(r => ({ key: r.type, value: r.value }))
      }
