<template>
  <div class="component-detail">
    <tabs>
      <tabx v-if="$store.state.user?.userRights.measureSchemeList" text="Параметры">
        <preserver-component v-bind="{ readOnly: !$store.state.user?.userRights.measureSchemeListEdit, saving, disabled: !hasChanges || loading, loading }" @saveClick="save()">
          <point-list-props v-bind="{ name: localItem.name, error: localError }" @change="onChanged" />
        </preserver-component>
      </tabx>
      <tabx v-if="$store.state.user?.userRights.measureSchemeList" text="Точки учета" :selected="true">
        <preserver-component v-bind="{ readOnly: !$store.state.user?.userRights.measureSchemeListEdit, saving, disabled: !hasChanges || loading, loading }" @saveClick="save()">
          <points-list v-bind="{ items, settings: localItem.settings }" @add="onAddPoints" @remove="onRemovePoints" />
        </preserver-component>
      </tabx>
      ...
      <tab text="Состояние">
        <state-list v-bind="{ id, type: $store.state.env.dbTypes.dbMeasureSchemaList }" />
      </tab>
      <tab text="Уведомления">
        <system-messages v-bind="{ objectId: id, objectType: $store.state.env.dbTypes.dbMeasureSchemaList }" />
      </tab>
      <tab text="Журнал">
        <journal-list
          v-bind="{ id, type: $store.state.env.dbTypes.dbMeasureSchemaList, periodStart: new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate() - 1, 0, 0, 0), periodEnd: new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate(), 23, 59, 59) }"
        />
      </tab>
    </tabs>
    <transition-group>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const items = computed(() => localItem.value.points.map(r => new Point(r)))
    ...
    function onRemovePoints(points: Array<number>) {
      localItem.value.points = localItem.value.points.filter(r => !points.includes(r.id))
      hasChanges.value = true
    }
