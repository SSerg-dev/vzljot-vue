<template>
  <div class="component-detail">
    <tool-bar>
      <div v-if="$store.state.user?.userRights.pollData" :class="['button', 'fas', 'fa-arrow-alt-circle-right', { disabled: !hasSelected || polling }]" title="Собрать данные" @click="poll()" />
      <div v-if="$store.state.user?.userRights.pollData" :class="['button', 'fas', 'fa-pause-circle', { disabled: !polling }]" title="Остановить" @click="cancel()" />
      <template v-if="$store.state?.user.userRights.measureSchemeList">
        <div v-if="$store.state.user?.userRights.pollData" class="separator" />
        <div :class="['button', 'fas', 'fa-plus-circle', { disabled: polling }]" title="Добавить..." @click="onAddClick" />
        <div :class="['button', 'fas', 'fa-times-circle', { disabled: !hasSelected || polling }]" title="Удалить" @click="onDeleteClick" />
        <template v-if="$store.state.user?.userRights.report">
          <div class="separator" />
          <div :class="['button', 'fas', 'fa-file-alt', { disabled: !hasSelected || polling || !hasReports }]" title="Сформировать отчет..." @click="reportClick" />
        </template>
      </template>
      <template v-slot:end>
        <div v-if="$store.state.user?.userRights.pollDataManual" :class="['button', 'fas', 'fa-cog', { disabled: polling }]" title="Настройки" @click="onSettingsClick()" />
        <div v-if="$store.state.user?.userRights.pollDataManual" class="separator" />
        <div :class="['button', 'fas', 'fa-filter', 'remove-sign', { disabled: !hasFilterChanges, right: !$store.state.user?.userRights.pollDataManual }]" title="Очистить фильтр" @click="onClearFilterClick" />
    ...
    <div class="table-grid" :style="`grid-template-columns: repeat(${columnsCount}, max-content)`">
      <header class="header sticky-first column" :style="`grid-template-columns: repeat(${firstColumnsCount - 1}, min-content)`">
        <span v-if="$store.state.user?.userRights.pollData" class="table-icon fas fa-cog" style="color: #596673" />
        <input v-if="$store.state.user?.userRights.measureSchemeList" type="checkbox" v-model="all" @change="changeAll" :disabled="polling" />
        Точка учета
      </header>
      ...
      <div v-for="(r, index) in localItems" :key="index" class="table-row">
        <span class="cell sticky-first column" :style="`grid-template-columns: repeat(${firstColumnsCount}, min-content)`">
          ...
          <input v-if="$store.state.user?.userRights.measureSchemeList || $store.state.user?.userRights.pollData" type="checkbox" v-model="r.checked" :disabled="polling" />
          <span :class="`fas fa-circle ${$store.state.env.statuses[r.state].class} table-icon`" />
        ...
        <span class="cell">{{ r.lastArchiveTime ? new Date(r.lastArchiveTime).toLocaleString() : '' }}</span>
        ...
        <span class="cell" v-for="(prop, propIndex) in customColumns" :key="'p' + propIndex">{{ r.custom[propIndex].value }}</span>
      </div>
    </div>
    <pager-component v-bind="pageInfo" @go="onChangePage" />
    <transition-group>
      <div v-if="isReport" style="display: flex; position: absolute; top: 0; bottom: 0; left: 0; right: 0; background: rgba(169, 175, 183, 0.7); padding: 15px; z-index: 5">
        <div style="flex: 1; flex-direction: column; width: 100%; position: relative">
          <report-component v-bind="reportData" @cancel="onReportCancel" @close="onReportClose" @reportLoaded="onReportLoaded" />
        </div>
      </div>
...
export default {
  ...
  computed: {
    reportData() {
      ...
      return rows.length > 0
        ? {
            ids: rows.map(r => r.id),
            objType: rows.length > 1 ? this.reportObjectTypes.listOfMeasureSchemes : this.reportObjectTypes.measureScheme,
            reportTypes: rows.length > 1 ? [reportTypes.summary] : rows[0].reportTypes,
            background: `rgba(169, 175, 183, ${this.reportLoaded ? '0.7' : '0'})`,
            cancellable: true
          }
        : null
    },
    ...
    filteredItems() {
      ...
      if (this.filter.periodStart) {
        ...
        rows = rows.filter(r => new Date(r.lastArchiveTime) >= new Date(periodStart.getFullYear(), periodStart.getMonth(), periodStart.getDate(), 0, 0, 0))
      }

      if (this.filter.periodEnd) {
        const periodEnd = new Date(this.filter.periodEnd)
        rows = rows.filter(r => new Date(r.lastArchiveTime) <= new Date(periodEnd.getFullYear(), periodEnd.getMonth(), periodEnd.getDate(), 23, 59, 59))
      }

      if (this.filter.node) {
        rows = rows.filter(r => r.node.fullName.toLowerCase().indexOf(this.filter.node.toLowerCase()) >= 0)
      }

      if (this.filter.point) {
        rows = rows.filter(r => r.name.toLowerCase().indexOf(this.filter.point.toLowerCase()) >= 0)
      }
      ...
      if (this.filter.consumer) {
        rows = rows.filter(r => r.consumer !== null && r.consumer.toLowerCase().indexOf(this.filter.consumer.toLowerCase()) >= 0)
      }
  ...
  methods: {
    ...
    onSi(message) {
      if (message && message.event === 'update' && this.$store.state.env.itemTypes[message.type].type === 'point') {
        this.dataItems.filter(r => r.id === message.item.id).forEach(r => (r.state = message.item.state))
      }
    },
    onReportLoaded() {
    ...
    async poll() {
      ...
      try {
        ...
        await this.$http.post(
          'pointList/poll',
          {
            ids: this.dataItems.filter(row => row.checked).map(row => row.id),
            settings
          },
