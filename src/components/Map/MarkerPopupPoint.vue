<template>
  <div class="wrapper">
    ...
    <div v-if="marker.data !== null" style="overflow-y: auto; max-height: 145px">
      <table v-for="(item, index) in marker.data" class="table-pattern" style="width: 100%" :key="index">
        <tr>
        ...
        <tr v-for="(value, index) in item.rows" :key="index">
          ...
          <td>
            <template v-if="value.value.indexOf('\r\n') !== -1">
              <div v-for="(event, index) in value.value.split('\r\n')" :key="index">{{ event }}</div>
            </template>
            <template v-else>
              {{ value.value }}
            </template>
